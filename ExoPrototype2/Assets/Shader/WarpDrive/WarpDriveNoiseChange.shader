Shader "Custom/WarDriveNoiseChange"
{
    Properties
    {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                /*for (y = -1; y <= 1; y++)
                {
                    for (x = -1; x <= 1; x++)
                    {
                        ...
                        float2 noiseOffset = float2(0.1, 0.3);
                        float2 noisePos = id * _NoiseScale + offset;
                        ...
                        float noise = cellNoise(noisePos, _CellSize);
                        float size = frac(noise * 123.32);
                        float warp = warp_tex(uvFrac - noiseOffset - float2(noise, frac(noise * 56.21)));
                        col += warp;*/
